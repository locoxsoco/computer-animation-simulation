...
Model Model::createIcosphereNI(int numSubdivisions) {
    ...
    std::vector<float> coords(3*verts.size());
    std::vector<float> norms(3*verts.size());
    for (unsigned int i = 0; i < verts.size(); i++) {
        coords[3*i  ] = float(verts[i][0]);
        coords[3*i+1] = float(verts[i][1]);
        coords[3*i+2] = float(verts[i][2]);
        norms[3*i  ] = -float(verts[i][0]);
        norms[3*i+1] = -float(verts[i][1]);
        norms[3*i+2] = -float(verts[i][2]);
    }

    return Model(coords, norms, tris);
}
